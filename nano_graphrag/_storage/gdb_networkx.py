    ...
    async def _leiden_clustering(self):
        ...
        node_communities = {
            k: v for k, v in sorted(
                dict(node_communities).items(),
                key=lambda item: (item[1][0]['level'], item[1][0]['cluster'], item[0])
            )
        }
        __levels = {k: len(v) for k, v in sorted(__levels.items(), key=lambda x: x[0])}
